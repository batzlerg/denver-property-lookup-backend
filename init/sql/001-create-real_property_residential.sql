...
ADD COLUMN property_address TEXT GENERATED ALWAYS AS (site_nbr || case when site_dir <> '' then ' ' else '' end || site_dir || ' ' || site_name || ' ' || site_mode || case when site_more <> '' then ' ' else '' end || site_mode) STORED;
